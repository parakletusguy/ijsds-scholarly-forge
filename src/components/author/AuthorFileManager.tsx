...
import { Upload, FileText, Calendar, RefreshCw, Download } from 'lucide-react';
...
export const AuthorFileManager = ({ articleId, submissionId }: AuthorFileManagerProps) => {
  ...
  const fetchFiles = async () => {
    try {
      // Use backend API — bypasses Supabase RLS entirely
  ...
  // Called by FileUpload after the backend has already created the file_version record.
  // We only need to update the article's manuscript_file_url and refresh the list.
  const handleFileUpdate = async (fileUrl: string | File) => {
    if (fileUrl instanceof File) return; // autoUpload=true so this won't happen

    setUploading(true);
    try {
      // Update the article's primary manuscript URL to the new version
